...
export class AuthService{
    ...
    signup(account: singupModel){
        return this.http.post<AuthResData>('http://0.0.0.0:8000/api/accounts/signup/',account)
    ...
    login(account: loginModel){
        return this.http.post<AuthResData>('http://0.0.0.0:8000/api/accounts/login/',account)
        .pipe(catchError(this.handleError),tap((res)=>{
            this.handleAuth(res);
        }))
    }
